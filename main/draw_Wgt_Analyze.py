...
path_Diff = r"E:\1Master_2\Paper_Grid\Pro_20211205-339\server-Wgt-HKST-HKPC-HKOH\HKOH-GEC-5.diff"
path_Ele = r"E:\1Master_2\Paper_Grid\Pro_20211205-339\server-Aug\HKOH-GEC.aug"
Save_Dir = r"E:\1Master_2\Paper_Grid\Pro_20211205-339\Fig\HKOH_Diff_Ele"
if (not os.path.exists(Save_Dir)):
    os.mkdir(Save_Dir)
[Head_Ele,data_Ele] = rf.open_aug_file_new(path_Ele)
[Head_Diff,data_Diff] = rf.open_aug_file_new(path_Diff)

Year,Mon,Day,Hour,LastT,deltaT = 2021,12,5,2,18,2
time = "UTC"
        ...
        for sat in data_Diff[time].keys():
            ...
            if sat[0] == "C":
                ...
                with open(Save_Dir + "\\" + sat[0] + ".txt",'a') as file1:
                    file1.write('%04f' % abs(data_Diff[time][sat][sys_type[sat[0]]]) + "   " + '%04f' % data_Ele[time][sat]["ELE"] + "\n")
            ...
            if sat[0] == "G":
                ...
                with open(Save_Dir + "\\" + sat[0] + ".txt",'a') as file2:
                    file2.write('%04f' % abs(data_Diff[time][sat][sys_type[sat[0]]]) + "   " + '%04f' % data_Ele[time][sat]["ELE"]+ "\n")
            ...
            if sat[0] == "E":
                ...
                with open(Save_Dir + "\\" + sat[0] + ".txt",'a') as file3:
                    file3.write('%04f' % abs(data_Diff[time][sat][sys_type[sat[0]]]) + "   " + '%04f' % data_Ele[time][sat]["ELE"]+ "\n")
