...
import math
site_list = ["REDU","BRUX","TIT2","KOS1","WSRT","PTBB"]
for site1 in site_list:
    time,year,mon,day,starttime,LastT,deltaT,count = "UTC",2023,1,29,4,21,11,7
    ...
    # mode_list = ["BNC-SEPARATE","BNC-CONTINUE"]
    # mode_list = ["500","10000"]
    site = Site
    ...
    while count_temp > 0:
        filename_list = [
                os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\SERVER_BNC7D\server_{:0>4}{:0>3}".format(year,29),site1+"-GEC3-FLOAT-30.flt"),
                os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\SERVER_BNC7D\server_{:0>4}{:0>3}".format(year,29),site1+"-GEC3-FIXED-30.flt"),
                # os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\SERVER_BNC\server_{:0>4}{:0>3}".format(year,doy_temp),site1+"-GEC3-FLOAT-30.flt"),
                # os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\CLIENT_BNC\client_{:0>4}{:0>3}".format(year,doy_temp),site1+"-GEC3-FIXED-30.flt"),
                # os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\CLIENT_BNC7D\client_{:0>4}{:0>3}".format(year,29),site1+"-GEC3-FIXED-30.flt"),
                os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\CLIENT_BNC7D\client_{:0>4}{:0>3}".format(2023,29),site1+"-GEC3-FIXED-30.flt"),
                    ]
        for i in range(len(mode_list)):
            ...
            with open(filepath,'rt') as f:
                for line in f:
                    if "#" not in line:
                        ...
                        if soweek not in all_data.keys() and (7 - count_temp) * 86400 <= soweek and (8 - count_temp) * 86400 > soweek:
    ...
    while count_temp > 0:
        ...
        #             ]
        filename_list = [
                os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\REF\{:0>3}".format(doy_temp),site1.lower() + "{:0>3}0.23zpd".format(doy_temp)),
                os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\REF\{:0>3}".format(doy_temp),site1.lower() + "{:0>3}0.23zpd".format(doy_temp)),
                os.path.join(r"E:\1Master_3\2_ZTD\Result_From_Server\REF\{:0>3}".format(doy_temp),site1.lower() + "{:0>3}0.23zpd".format(doy_temp)),
                    ]
    ...
    [XLabel,XTick,cov_Time,begT,LastT]=dr.xtick(time,year,mon,day,0,24*7,24)
    XLabel = []
    ddd = 29
    for i in range(8):
        XLabel.append("{:0>3}".format(ddd))
    ...
    for cur_mode in all_data.keys():
        ...
        for cur_day in all_data[cur_mode].keys():
            ...
            for cur_time in all_data[cur_mode][cur_day].keys():
                ...
                if (raw_plot_time >= begT and raw_plot_time <= begT + 168) and "{:.4f}".format(plot_time) in all_data_ref[cur_day][site1].keys():
