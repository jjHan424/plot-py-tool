...
for i in delay_list:
    cur_file = os.path.join(file_path_lstm,"HK-{}-LSTM-REAL.txt".format(i))
...
for i in delay_list:
    cur_file = os.path.join(file_path_arima,"HK-{}-ARIMA.txt".format(i))
...
for i in delay_list:
    cur_file = os.path.join(file_path_arima,"HK-{}-LSTM.txt".format(i))
...
for cur_type in data_plot.keys():
    ...
    for j in range(20):
        str_out = str_out+"{}:\"{:.2f}\",".format(j+1,np.mean([data_plot[cur_type]["G"][j],data_plot[cur_type]["E"][j],data_plot[cur_type]["C"][j]]))
        if j%2 != 0:
...
delay_list = range(1,21)
for i in delay_list:
    cur_file = os.path.join(file_path_lstm,"GER-{}-LSTM-NIGHT.txt".format(i))
    if i not in data_lstm.keys():
        data_lstm["G"][i],data_lstm["E"][i],data_lstm["C"][i] = [],[],[]
    with open(cur_file,"rt") as f:
        for line in f:
            ...
            data_lstm["C"][i].append(float(value[2]))
for i in delay_list:
    cur_file = os.path.join(file_path_arima,"GER-{}-ARIMA-NIGHT.txt".format(i))
    if i not in data_arima.keys():
        data_arima["G"][i],data_arima["E"][i],data_arima["C"][i] = [],[],[]
    with open(cur_file,"rt") as f:
        for line in f:
            ...
            data_arima["C"][i].append(float(value[2]))
for i in delay_list:
    cur_file = os.path.join(file_path_arima,"GER-{}-RAW-NIGHT.txt".format(i))
    if i not in data_raw.keys():
        data_raw["G"][i],data_raw["E"][i],data_raw["C"][i] = [],[],[]
    with open(cur_file,"rt") as f:
        for line in f:
...
[label.set_fontsize(xtick_size) for label in labels]
[label.set_fontname('Arial') for label in labels]
axP[0][0].set_title("DAY",font_title)
axP[0][1].set_title("NIGHT",font_title)
# axP[1][0].set_ylabel('Ionospheric errors (cm)',font_label)
# axP[2][0].set_xlabel('Prediction duration (min)',font_label)
# plt.savefig("/Users/hanjunjie/Desktop/Image-1/GER_DAY_VS_NIGHT.jpg",dpi = 300)
plt.show()