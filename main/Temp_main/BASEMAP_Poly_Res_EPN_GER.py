...
font_title = {'family' : 'Arial', 'weight' : 400, 'size' : 15}
...
ax.set_title("With preprocessiong",font_title)
box = ax.get_position()
...
#     map.scatter(crd_data[cur_site]["BLH"][1],crd_data[cur_site]["BLH"][0],marker='v',s=100,facecolor='#00BFFF',edgecolor='k', linewidth=0.3)
...
#Test Residual
resfile = "/Users/hanjunjie/Master_3/1-IUGG/RESULT/ResidualPoly/respoly_NEW.txt"
res_data = {}
with open(resfile,'rt') as f:
    for line in f:
        value = line.split()
        if value[0] not in res_data.keys():
            res_data[value[0]] = []
        res_data[value[0]].append(float(value[5]))
x,y=np.array(lat_list),np.array(long_list)
data_res = {}
mean_temp = 0
for cur_site in crd_data:
    data_res[cur_site] = (np.mean(res_data[cur_site]))
    mean_temp = mean_temp + np.mean(res_data[cur_site])
# print(data_res)
# max_res = max(data_res.values())
# min_res = min(data_res.values())
max_res = 4.0
min_res = 1.50
for cur_site in crd_data:
    heat_value = data_res[cur_site]
    norm_heat_value =(heat_value - min_res)/(max_res - min_res)
    heat_color = plt.cm.jet(norm_heat_value)
    map.scatter(crd_data[cur_site]["BLH"][1],crd_data[cur_site]["BLH"][0],marker='v',s=100,facecolor=heat_color,edgecolor='k', linewidth=0.3)
    texts.append(plt.text(crd_data[cur_site]["BLH"][1],crd_data[cur_site]["BLH"][0],cur_site,fontdict=font_text))
adjust_text(texts)
ax = fig.add_subplot(122)
ax.set_title("Without preprocessiong",font_title)
box = ax.get_position()
ax.set_position([box.x0, box.y0 - box.y0*1.5, box.width, box.height])
map = Basemap(llcrnrlon=3.2, llcrnrlat=48.9, urcrnrlon=10.7, urcrnrlat=53.4, resolution='c',projection='cyl')
...
map.readshapefile('/Users/hanjunjie/Master_3/1-IUGG/CRDSITE/shp/gadm36_BEL_shp/gadm36_BEL_0','states',drawbounds=True)
#Test Residual
resfile = "/Users/hanjunjie/Master_3/1-IUGG/RESULT/ResidualPoly/respoly_OLD.txt"
res_data = {}
with open(resfile,'rt') as f:
    for line in f:
        value = line.split()
...
for cur_site in crd_data:
    ...
    texts.append(plt.text(crd_data[cur_site]["BLH"][1],crd_data[cur_site]["BLH"][0],cur_site,fontdict=font_text))
adjust_text(texts)
norm = Normalize(vmin = min_res,vmax=max_res)
cax = plt.axes([0.25,0.143,0.5,0.02]) #left down width height
...
cb.set_ticklabels([1.5,2,2.5,3,3.5,4],fontsize=15,fontname="Arial")
# map.pcolormesh(x, y, data_res, cmap='jet', zorder=0,vmin = 0,vmax=5)
...
plt.tight_layout()
# plt.savefig("/Users/hanjunjie/Desktop/Image-1/EPN_SITE_DCB_RESDUALS.jpg",dpi=300)
plt.show()
# plt.clf()
plt.close()
