...
#Diff-EleSun
file_list = ["/Users/hanjunjie/Master_3/1-IUGG/ResFromServer/PLOT_HK_ROTI_IONO.txt"]
mode_list = ["Net. A","Net. B","Net. C"]
# filename = r"E:\1Master_2\Paper_Grid\Res_FromServer_New\Fig\Ele_Wgt-New\diff-meanele.txt"
Diff,ROTI,Time,Time_ele = {},{},{},{}
abc=[0.00919,0.10244,-0.09832]
for i in range(len(file_list)):
    filename = file_list[i]
    if mode_list[i] not in Diff.keys():
        Diff[mode_list[i]],ROTI[mode_list[i]],Time[mode_list[i]],Time_ele[mode_list[i]] = [],[],[],[]
    with open(filename,'rt') as f:
        for line in f:
            ...
            ROTI[mode_list[i]].append(float(value[0]))

figP,axP = plt.subplots(1,1,figsize=(6,3.5),sharey=False,sharex=True,frameon = False)
i=0
x = np.array(ROTI[mode_list[i]])
x = x[x>=0.8]
y = (abc[0]*x*x+abc[1]*x+abc[2])*100+1.04
axP.plot(x,y,linewidth=3.0,linestyle='--',c='k')
# axP.legend([r"$y = a \times \sin (x) + \frac{b}{{\sin (x)}} + c$"],prop = font_label)
axP.legend([r"$y = a{x^2} + b{x} + c$"],prop=font_legend,
...
axP.scatter(ROTI[mode_list[i]],Diff[mode_list[i]],s=5,c="#0099E5")
...
axP.set_position([box.x0, box.y0+box.height*0.10, box.width, box.height*0.95])
plt.savefig("/Users/hanjunjie/Desktop/Image-1/Diff_VS_ROTI.jpg",dpi=300)
plt.show()